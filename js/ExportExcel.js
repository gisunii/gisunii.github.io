...
function htmlToExcel(titleinfo,endinfo,GridPanel_ID, ColumnWidth, Border) {
	
    try {
        if (navigator.userAgent.indexOf("MSIE") > 0) {
            ToExcel_IE(titleinfo,endinfo,GridPanel_ID, ColumnWidth, Border);
        }
        else {
            ToExcel_FF(titleinfo,endinfo,GridPanel_ID, Border);
        }
        ColumnWidth = null;
    } catch (e) {
        alert(e.description)
...
function ToExcel_IE(titleinfo,endinfo,GridPanel_ID, ColumnWidth, Border) {
    ...
    //过滤elDiv内容
    var elDivStr = titleinfo + elDiv.innerHTML+endinfo;
    
    //elDivStr = elDivStr.replace(/<td/g, "<td style='border:0.5px solid;'");
...
//        oSheet.Columns(parseInt(ColumnWidth[i].toString().split(",")[0])).ColumnWidth = parseInt(ColumnWidth[i].toString().split(",")[1]);
//    }
...
var ToExcel_FF = (function() {
    ...
    return function(titleInfo, endInfo, tableID, border) {
        var elDiv = $('#' + tableID);

        //过滤elDiv内容
        var elDivStr =titleInfo + elDiv.prop("outerHTML") + endInfo;
        elDivStr = replaceHtml(elDivStr, "<a", ">");
        elDivStr = replaceHtml(elDivStr, "</a", ">");
        elDivStr = replaceHtml(elDivStr, "<img", ">");
        //设置table的border
